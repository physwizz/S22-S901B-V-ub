...
static const struct is_cis_log log_2la[] = {
	...
	{I2C_READ, 16, 0x0702, 0, "fll_shifter"},
	{I2C_READ, 16, 0x0704, 0, "cit_shifter"},
	/* 2000 page */
...
int sensor_2la_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		...
		cit_shifter = (u16)sensor_cis_get_duration_shifter(cis, input_duration);
	...
	/* CIT shifter */
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = is_sensor_write16(client, 0x0704, cit_shifter);
		if (ret < 0)
			goto p_err_i2c_unlock;
	}
