						...
						cis_shared_data *cis_data)
{
	...
	/* for hyperlapse night mode, increase the time of processing one frame calculation */
	if (cis_data->is_data.scene_mode == AA_SCENE_MODE_HYPERLAPSE) {
		cis_data->min_sync_frame_us_time = cis_data->request_min_frame_us_time;
		dbg_sensor(1,"[%s] min_sync_frame_us_time(%d)\n", __func__, cis_data->min_sync_frame_us_time);
	}
#endif
	/* 3. FPS calculation */
